...
function updateAllClientPercentages(val) {
    lastMainAllocationPct = val;
    clientMarginList.forEach((client, idx) => {

        clientPercentages[idx] = val;
    });
    renderAllocationRows();
}
...
async function generateCSV() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            a.download = `ClientWiseTradeFiles_${selectedDate || fallbackDate}.zip`;
...
async function saveLotsExcel() {
    ...
    clientMarginList.forEach((client, idx) => {
        let rowData = {
            ...
            'Percentage': clientPercentages[idx] || 100,
            'Total Margin': client.TotalMargin,
            'Strategies': []
        };
        
        marginFilters.forEach((mf, filterIdx) => {
            let key = `${idx}-${filterIdx}`;
            let input = document.querySelector(`.lot-input[data-client="${idx}"][data-filter="${filterIdx}"]`);
            let val = input ? parseInt(input.value) : 0;
            
            rowData.Strategies.push({
                'Strategy': mf.excelStrategy || mf.strategy,
                'Index': mf.index,
                'Lot': val
            });
        });
    ...
    try {
        const slPercent = lastMainAllocationPct || '100';
        const response = await fetch('/save_lots_excel', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                date: selectedDate,
        ...
        if (response.ok) {
            ...
            const a = document.createElement('a');
            a.href = url;
            const filenameSuffix = slPercent || '100';
            a.download = `ClientWiseLots_${filenameSuffix}.xlsx`;
...
let lastAllocData = null;

function moveColumn(idx, direction) {
    const newIdx = idx + direction;
    if (newIdx < 0 || newIdx >= marginFilters.length) return;
    
    // Swap in marginFilters
    [marginFilters[idx], marginFilters[newIdx]] = [marginFilters[newIdx], marginFilters[idx]];
    
    // Re-render
    renderAllocationTable(lastAllocData, true);
}

function reorderColumns(fromIdx, toIdx) {
    if (fromIdx === toIdx) return;
    const item = marginFilters.splice(fromIdx, 1)[0];
    marginFilters.splice(toIdx, 0, item);
    renderAllocationTable(lastAllocData, true);
}
...
function getDayBasedConfig(entryDay) {
    if (entryDay === 'Monday' || entryDay === 'Wednesday' || entryDay === 'Friday') {
        return {
            ...
            'DYNAMIC SL': [
                { type: 'N.Expiry', index: 'SENSEX', wh: 'W.H', percent: 0 },
                { type: 'N.Expiry', index: 'BANKNIFTY', wh: 'W.O.H', percent: 0},
                { type: 'N.Expiry', index: 'NIFTY', wh: 'W.H', percent: 15 }
            ]
        };
    } else if (entryDay === 'Tuesday') {
        return {
            'INDEXMOVE': [
                { type: 'Expiry', index: 'NIFTY', wh: 'W.O.H', percent: 15 }
            ],
            'DYNAMIC SL': [
                { type: 'N.Expiry', index: 'SENSEX', wh: 'W.H', percent: 0 },
                { type: 'N.Expiry', index: 'BANKNIFTY', wh: 'W.O.H', percent: 0 },
                { type: 'Expiry', index: 'NIFTY', wh: 'W.H', percent: 15 }
            ],
            'ITM SH': [
                { type: 'Expiry', index: 'NIFTY', wh: 'W.H', percent: 10, excelStrategy: 'I_2T23' }
            ],
            'TO': [
                { type: 'Expiry', index: 'NIFTY', wh: 'W.H', percent: 50 }
            ]
        };
        ...
        return {
            ...
            'DYNAMIC SL': [
                { type: 'Expiry', index: 'SENSEX', wh: 'W.H', percent:0 },
                { type: 'N.Expiry', index: 'BANKNIFTY', wh: 'W.O.H', percent: 0 },
                { type: 'N.Expiry', index: 'NIFTY', wh: 'W.H', percent: 15 }
            ],
            '2T3 TO': [
            ...
            'TO': [
                { type: 'Expiry', index: 'SENSEX', wh: 'W.H', percent: 50 }
            ]
        };
    }
    return {};
...
function renderAllocationTable(data, isReorder = false) {
    lastAllocData = data;
    ...
    if (!isReorder || marginFilters.length === 0) {
        marginFilters = [];
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (i === 0) {
            ...
            mainSelect.style.cssText = 'border:1px solid #000;padding:2px;font-size:11px;width:55px;background:#fff;cursor:pointer;border-radius:3px;';
            ...
            mainSelect.onchange = function() {
                if (this.value) updateAllClientPercentages(parseInt(this.value));
            };
            thPercent.appendChild(mainSelect);
        } else {
    ...
    marginFilters.forEach((mf, idx) => {
        ...
        th1.style.whiteSpace = 'nowrap';
        th1.textContent = mf.index;
        th1.style.cursor = 'grab';
        th1.draggable = true;
        
        th1.ondragstart = (e) => {
            e.dataTransfer.setData('text/plain', idx);
            th1.style.opacity = '0.4';
        };
        ...
        if (showMarginUsed) {
            ...
            marginFilters.forEach((mf, filterIdx) => {
                ...
                if (lotVal > 0 && marginValue > 0 && totalMargin > 0) {
                    let effectiveTotalMargin = totalMargin * percentFactor;
                    let marginUsedPercent = (marginValue * lotVal) / effectiveTotalMargin * 100;
